...
// TODO: finish tests
const useTestHook = <K>(mapper: (value: number | undefined) => K) => {
  const { state, setState, setRefresh } = useCommonState<number>()
  const mapped = useMapCommonState(state, mapper)

  return { setState, setRefresh, state, mapped }
}
...
describe('useCommonState hook', () => {
  ...
  it('check setRefresh', async () => {
    const mapper = (value: number | undefined) => (value === undefined ? undefined : `_${value + 1}`)
    const { result, waitForNextUpdate } = renderHook(() => useTestHook(mapper))
